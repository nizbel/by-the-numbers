	...
	void Update () {
		if (mainMenu.GetState() == MainMenuController.MAIN_MENU) {
			...
			this.GetComponent<RectTransform>().sizeDelta = Vector2.Lerp(this.GetComponent<RectTransform>().sizeDelta, new Vector2(
				GameController.GetCamera().WorldToScreenPoint(new Vector3(screenSize*MODE_MENU_WIDTH, 0, 0)).x / mainMenu.GetCanvas().scaleFactor, Screen.height*MODE_MENU_HEIGHT / mainMenu.GetCanvas().scaleFactor),
			...
			if (Mathf.Abs(this.transform.localPosition.magnitude - modeMenuPosition.magnitude) < APPROXIMATION_CONSTANT
				...
				GameController.GetCamera().WorldToScreenPoint(new Vector3(screenSize*MODE_MENU_WIDTH, 0, 0)).x / mainMenu.GetCanvas().scaleFactor, Screen.height*MODE_MENU_HEIGHT / mainMenu.GetCanvas().scaleFactor).magnitude)) < APPROXIMATION_CONSTANT) {
