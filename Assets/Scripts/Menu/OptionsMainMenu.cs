	...
	void Update () {
		if (mainMenu.GetState() == MainMenuController.MAIN_MENU) {
			...
			this.GetComponent<RectTransform>().sizeDelta = Vector2.Lerp(this.GetComponent<RectTransform>().sizeDelta, new Vector2(
				GameController.GetCamera().WorldToScreenPoint(new Vector3(screenSize*0.9f, 0, 0)).x / mainMenu.GetCanvas().scaleFactor, Screen.height*0.9f / mainMenu.GetCanvas().scaleFactor), Time.deltaTime*MainMenuController.MENU_TRANSFORMATION_SPEED);
			...
			if (Mathf.Abs(this.transform.localPosition.magnitude) < 0.5f 
				...
				GameController.GetCamera().WorldToScreenPoint(new Vector3(screenSize*0.9f, 0, 0)).x / mainMenu.GetCanvas().scaleFactor, Screen.height*0.9f / mainMenu.GetCanvas().scaleFactor).magnitude) < 0.5f) {
				
				this.transform.localPosition = Vector3.zero;
				this.GetComponent<RectTransform>().sizeDelta = new Vector2(
					GameController.GetCamera().WorldToScreenPoint(new Vector3(screenSize*0.9f, 0, 0)).x / mainMenu.GetCanvas().scaleFactor, Screen.height*0.9f / mainMenu.GetCanvas().scaleFactor);
