...
SubShader {
	...
	Pass
	{
		...
		fixed4 frag(v2f IN) : SV_Target
		{
			...
			float4 initialBurn = float4(0, 0, tint, alpha);
			float4 endBurn = float4(tint, 0, tint, alpha);
			...
			else if (any(dissolveColor.rgb - _DissolveAmount < 0.08)) {
				return initialBurn;
			}
			else if (any(dissolveColor.rgb - _DissolveAmount < 0.12)) {
				return endBurn;
			}
